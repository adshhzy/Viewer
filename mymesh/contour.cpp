...
void Contour::ReScale(double lscale,double *pcenter){
...
void Contour::ComputeMat2VerticesLoop(vector<vector<int>>&verticesloops, vector<vector<double>>&verticesSeqs, vector<int>&loops2mats){
    vector<vector<int>>edgesloops;
    ComputeMat2VerticesLoop(verticesloops,edgesloops,verticesSeqs,loops2mats);
}

void Contour::ComputeMat2VerticesLoop(vector<vector<int>>&verticesloops, vector<vector<int>>&edgesloops, vector<vector<double>>&verticesSeqs, vector<int>&loops2mats){
...
bool CrossSections::ReadCrossSections_Ctr(string filename){
    ...
    ReadCrossSections(planeinfo,plane2vertices,plane2edges,plane2edgesMat);
...
void CrossSections::ReScale_uniform(double lscale, bool isMoveCent){

    double largestdis = -1;
    for(int i=0;i<3;++i)largestdis = max(largestdis,cornerdist[i]);
    largestdis = largestdis/2 / lscale;
    //cout<<centers[0]<<"  "<<centers[1]<<"  "<<centers[2]<<"   "<<largestdis<<endl;

    largestdis = 1/largestdis;

    for(auto &a:CSs)a.ReScale(largestdis,midpoint.data());
}
...
void CrossSections::CalSelfProperty(){
    ...
    for(int j=0;j<3;++j)cornerdist[j] = rightcorner[j]  - leftcorner[j];
    ...
    if(1){
        int newind = 0;
        unordered_map<int, int>mapper;
        for(auto a:matts)mapper[a] = newind++;
        for(auto &a:CSs)for(auto &b:a.m1)b = mapper[b];
        for(auto &a:CSs)for(auto &b:a.m2)b = mapper[b];
        for(auto &a:CSs)a.n_materials = n_materials;
    }
...
void CrossSections::GetProperty(vector<double>&mid,vector<double>&leftc,vector<double>&rightc){
    CalSelfProperty();
    mid = midpoint;
    leftc = leftcorner;
    rightc = rightcorner;
...
void CrossSections::Stackup(vector<double>&points, vector<uint> &edges, vector<double>&enormal){
    ...
    //for(int i=0;i<n_CrossSec;i++)for(int j=0;j<CSs[i].n_vertices*3;++j)points.push_back( CSs[i].vertices[j]);
    for(int i=0;i<n_CrossSec;i++)points.insert(points.end(), CSs[i].vertices.begin(),CSs[i].vertices.end());
    edges.clear();
    for(int i=0;i<n_CrossSec;i++)for(int j=0;j<CSs[i].n_edges*2;++j){
        edges.push_back( CSs[i].edges[j]+acc_nver[i]);
    }
    //enormal.clear();
    //for(int i=0;i<n_CrossSec;i++)CSs[i].GetEdgesNormal(enormal,true);
    enormal.clear();
    for(int i=0;i<n_CrossSec;i++)enormal.insert(enormal.end(),CSs[i].edge_normal.begin(),CSs[i].edge_normal.end());
