...
void Surface::WeightColor(const double thres,const vector<double>&weights){
    ...
    weighted_color.resize(n_para*4);
    for(int i =0;i<n_para;++i){
        auto a = fmin(fabs(weights[i]),(double)thres);
        if(!useHSV){
            a=min(1.,a/(double)thres);
            if(0){
                a=a*2-1;
                weighted_color[i*4] = uchar(min(255.,jet_red(a)*255));
                weighted_color[i*4+1] = uchar(min(255.,jet_green(a)*255));
                weighted_color[i*4+2] = uchar(min(255.,jet_blue(a)*255));
            }else{
                COLOUR jetcolor = GetColour(a,0,1);
                weighted_color[i*4] = uchar(min(255.,jetcolor.r*255));
                weighted_color[i*4+1] = uchar(min(255.,jetcolor.g*255));
                weighted_color[i*4+2] = uchar(min(255.,jetcolor.b*255));
            }
...
void ThresholdColoring(double degree,uchar *p_rgb){
    if(degree == -10000.){//10000
        ...
        if(1)ThresholdColoring(degree,0.8,0.8,p_rgb);
        else{
