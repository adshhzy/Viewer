...
private:
    ...
    vector< uint >edges2edges;
    vector< unsigned long >edges2edges_accumulate;
...
public:
    ...
    bool ImportCurve_Obj(string filename);
    bool ImportCurve_curve(string filename, int n_readSeg = -1);
    bool WriteCurveNet(string filename);
    bool ValidationCheck();

    bool AddCurve(Curve& a);

    bool ImportCurve_Cindy(string filename);
    bool ImportCurve_curvepp(string filename);
    bool ExportCurve_curvepp(string filename);
    void Set_EdgeNormal(vector<uint>&eind, vector<double>&vnormal);
    void PortionExport(double cutpercentage,int method, vector<double>&points, vector<uint>&newedges, vector<uint>&vind, vector<uint>&eind);
    void ExportSegments(vector<int> &pickSegs, vector<double>&points, vector<uint>&newedges, vector<uint>&vind, vector<uint>&eind);
    bool WriteSegments(vector<int> &pickSegs, string filename);


    void ReScale_Uniform(double &lscale, double *pcenter);
    void RescaleUniform(double lscale = 1, bool isMoveCent = true);
...
public:
    ...
    inline uint* ee_begin(uint e_ind){ return &(edges2edges[edges2edges_accumulate[e_ind]]); }
    inline uint* ee_end(uint e_ind){ return &(edges2edges[edges2edges_accumulate[e_ind+1]]); }
    inline uint ee_num(uint e_ind){ return edges2edges_accumulate[e_ind+1]-edges2edges_accumulate[e_ind]; }
};
...
public:
    int numofCurv;
    vector<Curve>Curv_buffer;

    void BatchRead(vector<string> &fnames);
